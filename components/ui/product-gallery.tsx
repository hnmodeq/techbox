  ...
  return (
    <div className="mt-8 overflow-hidden rounded-[var(--corner-radius)] border-[length:var(--border-size)] border-[var(--border-color)] shadow-[var(--shadow-size)]">
      <div className="relative aspect-[4/3] bg-[var(--muted-background)]">
        <Image src={current} alt={`${title} - تصویر ${index + 1}`} fill sizes="(min-width:768px) 768px, 100vw" className="object-contain" />
        {clean.length > 1 && (
      ...
      {clean.length > 1 && (
        <div className="flex gap-2 overflow-x-auto bg-[var(--card-background)] p-3">
          {clean.map((src, i) => (
            <button key={src} type="button" onClick={() => setIndex(i)} className={`relative h-16 w-20 shrink-0 overflow-hidden rounded-[var(--corner-radius)] border ${i === index ? 'border-[var(--shop)]' : 'border-[var(--border-color)]'}`}>
