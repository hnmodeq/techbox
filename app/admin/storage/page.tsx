...
type StorageFolder = { name: string; prefix: string; count: number; size: number };
type StorageFile = {
  pathname: string;
  name: string;
  url: string;
...
function buildTree(files: StorageFile[], prefix: string): TreeFolder {
...
function TreeView({
  ...
  const renderFile = (file: StorageFile, depth: number) => {
...
function StorageContent() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    fetch(`/api/admin/storage?prefix=${encodeURIComponent(prefix)}`, { cache: "no-store" })
      .then(async (r) => {
        const body = await r.json();
        if (!r.ok) throw new Error(body?.message || body?.error || "blob_list_failed");
        return body as StorageResponse;
      })
