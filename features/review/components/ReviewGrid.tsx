...
export default function ReviewGrid(){
 ...
 <span className="absolute bottom-3 left-3 rounded-[var(--tb-radius-full)] bg-[color-mix(in oklch, black 60%, transparent)] px-2 py-1 tb-text-sm text-[var(--tb-warning)] backdrop-blur-[var(--tb-blur-sm)]">{stars(rating)} {rating.toFixed(1)}</span>
 </Link>
 <div className="p-4 flex flex-col flex-1">
 <h3 className=" tb-text-md line-clamp-2 min-h-[56px]">
 <Link href={`/review/${r.slug}`} className="hover:text-[var(--tb-review)]">{r.title}</Link>
 </h3>
 <p className="tb-text-sm text-muted-foreground line-clamp-2 mt-2 flex-1">{r.excerpt}</p>
 ...
 <div>
 <div className="tb-text-sm ">{r.author.name}</div>
 <div className="tb-text-sm text-muted-foreground">{r.author.role || "نویسنده"}</div>
 </div>
 </div>
 <div className="tb-text-sm text-muted-foreground text-left ">
